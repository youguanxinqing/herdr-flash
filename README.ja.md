...
- **Herdr 0.9.0 を推奨**。最低対応バージョンは **0.7.4**
- Rust/Cargo（現状インストールはソースからのビルド。ビルド済みのリリース成果物はまだありません）
- クリップボードコマンドは不要：`y` は OSC 52 で書き込み、Herdr が接続先のターミナルへ中継する。
  そのため `herdr --remote` のペインでも、リモートホストではなく手元のマシンにコピーされる。

2026-09-16 時点では、Herdr 0.9.0 を主な検証対象としています。クライアントとサーバーの両方が
0.9.0、接続クライアントが 1 つの環境で、ピッカーの起動、分割/zoom レイアウト、Escape による
終了と後処理を実機確認済みです。現在の Flash コードには、0.9.0 での起動遅延への対策が含まれます。
0.7.4 と 0.8.2 との互換性も維持し、関連 API の動作確認とフォールバックのテストを行っていますが、
今回の対策を適用した旧バージョンでの起動性能は未計測です。0.9.0 に複数のクライアントを接続した
場合、レイアウト更新待ちによる遅延が残ることがあります。
詳細は[互換性と遅延の記録](docs/bugs/herdr-090-hidden-layout-latency.md)を参照してください。
...
`y` は `pbcopy`/`xclip` を起動しない。自分のペインへ OSC 52 シーケンスを書き、それを Herdr が
接続先のターミナルへ中継する。OSC 52 はターミナルの標準であって、Herdr が文書化したプラグイン
API ではない。つまり Herdr 側が互換性を壊したと考えないまま動かなくなることがある。issue を
立てる前に、次の 2 つでどちら側が壊れたか切り分けられる。

**1. そもそも経路は生きているか。** 任意の Herdr ペインのシェルで実行する:

```sh
printf '\033]52;c;b3NjNTItd29ya3M=\a'
```

必ずそのシェルに直接打ち込むこと。パイプ、`$(...)` での取り込み、ターミナル内の AI アシスタント
に代行させる、のいずれも出力がそこで横取りされ、シーケンスがターミナルまで届かない。

画面には何も出ない。仕様上、不可視のシーケンスだから。そのままどこかに貼り付ける。

- **`osc52-works` が貼れる** — 経路は正常で、バグはこのプラグイン側。`herdr --version` と使用中
  のターミナルを添えて
  [issue を立ててほしい](https://github.com/youguanxinqing/herdr-flash/issues)。
- **何も貼れない、または以前の内容のまま** — 経路のどこかが OSC 52 を中継していない。このプラグ
  インをどう直しても解決しない。手順 2 で報告先が分かる。

**2. どちらが拒んでいるか。** Herdr の**外**のターミナルタブで、同じ `printf` を実行する。

- **Herdr の外では動き、中では動かない** — Herdr がペイン発のシーケンスを中継していない。Herdr
  へ issue を立てる価値がある。ペイン側の OSC 52 は未文書化なので、バグ報告であると同時に要望
  でもある。
- **どちらでも動かない** — ターミナル側で OSC 52 の書き込みが無効になっている。Ghostty では
  `clipboard-write`（`allow`/`ask`/`deny`）。大半のターミナルに同等の設定がある。

ピッカーのステータス行に `copy failed · …` が出るのは、書き込み自体が失敗したときだけ。シーケン
スを受け取ったうえで黙って捨てるターミナルは、成功と区別がつかない。クリップボード用のプロセス
を起動しないことの代償であり、手順 1 はそれを確かめるためにある。
