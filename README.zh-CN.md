...
截至 2026-09-16，Herdr 0.9.0 是当前主要测试版本，实测环境为客户端和服务端均为 0.9.0、
单客户端连接。已验证选择器启动、分屏/zoom 布局和 Escape 退出清理，当前 Flash 代码已包含
0.9.0 选择器启动延迟的适配。仍保留对 0.7.4、0.8.2 的兼容，已核对相关 API 行为并测试回退路径，
但尚未在这两个旧版本上实测本次适配后的启动性能。0.9.0 多客户端同时连接时，选择器仍可能因
等待布局更新而出现延迟。详见[兼容性与延迟记录](docs/bugs/herdr-090-hidden-layout-latency.md)。
